...
- `ImportID` - Type: `string` - A unique import ID to be associated with any run import jobs. This allows for different import configurations (for example, imports from different Entra ID tenants) to be run concurrently.
...
- `User` - Type: `object` - Contains all Entra ID to Hornbill mappings, and user configuration.
  ...
  - `Manager` - Type: `object` - Contains three properties, Import actions to perform on the Manager field of the discovered user records:
    - `Action` - Type: `string` - Can be:
      - `Create`, `Update`, or `Both` - any of these (case-insensitive) can be used to activate manager association
...
The "Output" element appears inside the User.Actions array, and it defines the name of a variable that stores the result of that transform action, so it can be referenced elsewhere in your mapping.

Each object in Actions runs a transform (Trim, Replace, or Regex) against a source Value, and Output is where the transformed result gets written to.
...
},
{
  "Action": "Replace",
  "Value": "{{.mobile}}",
  "Options": { "replaceFrom": "0", "replaceWith": "+44" },
  "Output": "phone"
},
{
  "Action": "Regex",
  "Value": "{{.telephoneNumber}}",
  "Options": { "regexValue": "([0-9])\\w+/g" },
  "Output": "areaCode"
}
```

The Actions array holds a list of transform actions that can be performed on mapped field values, where Action is one of Trim, Replace or Regex — Trim removes leading/trailing whitespace, Replace swaps part of a string for another string, and Regex searches the value using a regular expression and returns the match.

In the example json file...

- The department action trims whitespace off {{.department}} and stores the cleaned-up value in a new variable called department.
- The phone action replaces a leading 0 with +44 in {{.mobile}} and stores it as phone.
- The areaCode action extracts digits from {{.telephoneNumber}} via regex and stores the result as areaCode.

Once an Output name is set, you can reference it in your AccountMapping or ProfileMapping fields as {{.department}}, {{.phone}}, {{.areaCode}}, etc. instead of the raw Entra ID field — effectively giving you cleaned/transformed versions of the data to map into Hornbill fields, rather than the raw values straight from Entra ID.
