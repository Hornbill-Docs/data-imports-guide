...
- `AzureConf` - Type: `object` - The information that the utility needs to query Entra ID.
  ...
  - `UserID` - Type: `string` - The Entra ID field that contains the User ID.
...
- `User` - Type: `object` - Contains all Entra ID to Hornbill mappings, and user configuration.
  ...
  - `UserDN` - Type: `string` - The user distinguished name, mapped from the source data.
  ...
  - `Image` - Type: `object` - Contains several properties, Import actions to perform on profile images of the discovered user records:
    ...
    - `InsecureSkipVerify` - Type: `boolean` - Skip insecure certificate verification
  ...
  - `Org` - Type: `array` - An array of objects containing several properties of Organisational Unit information to associate the imported user with:
    - `Action` - Type: `string` - Can be `Create`, `Update`, or `Both`. 
    - `Value` - Type: `string` - The mapped name of the Organisational Unit.
    - `MemberOf` - Type: `string` - This field references the "memberOf" attribute of the user object in your Entra ID directory. When the distinguished of a group is specified in this field, the utility will only allow the group association in Hornbill if the group DN exists in the`memberOf` attribute of the user object being processed. i.e. only allow the Hornbill group association if the user object is a member of this directory group.