...
As the Asset Relationship Import utility supports the import of asset data from many different data sources, it must therefore also support many different KeySafe Key types:
...
Keys of this type require a Certero API Key to be created against a user account that has permissions to fetch assets before the details can be stored in KeySafe, and eventually be used by the Asset Relationship Import Utility.
