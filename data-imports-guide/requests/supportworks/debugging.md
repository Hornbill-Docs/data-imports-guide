...
- ``[ERROR] **Database Query Error: write tcp xx.xx.xx.xx:xxxx->xx.xx.xx.xx:xxxx: wsasend: An existing connection was forcibly closed by the remote host.**`` - This is most likely due to the incorrect database driver being specified in the conf file. Check you are using the right database driver for the database which stores your Supportworks data. It may also be prudent to check the username and password are correct too.
